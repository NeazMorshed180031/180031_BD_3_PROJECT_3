...
# It transforms the good data using the PCA fit above
reduced_data = pca.transform(good_data)

# It transforms the sample log-data using the PCA fit above
pca_samples = pca.transform(samples)

# It creates a DataFrame for the reduced data
reduced_data = pd.DataFrame(reduced_data, columns = ['Dimension 1', 'Dimension 2'])

# It displays sample log-data after applying PCA transformation in two dimensions
display(pd.DataFrame(np.round(pca_samples, 4), columns = ['Dimension 1', 'Dimension 2']))

# It creates range of clusters 
range_n_clusters = list(range(2,11))

for n_clusters in range_n_clusters:
    # It applies your clustering algorithm of choice to the reduced data 
    clusterer = KMeans(n_clusters=n_clusters).fit(reduced_data)

    # It predicts the cluster for each data point
    preds = clusterer.predict(reduced_data)

    # It finds the cluster centers
    centers = clusterer.cluster_centers_

    # It predicts the cluster for each transformed sample data point
    sample_preds = clusterer.predict(pca_samples)

    # It calculates the mean silhouette coefficient for the number of clusters chosen
